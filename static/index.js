...
const codemirrorLinkStyles = r$2`
 ...
 * TODO DOCS
 */
class BcDatalogEditor extends s {
...
class BcTokenEditor extends s {
  ...
  render () {
    return p`
      ...
      ${this._blocks.map((block, index) => p`
 ...
 * TODO DOCS
 */
class BcAuthorizerResult extends s {
    ...
    return p`
      <div>${
        Object.keys(facts_map).map(function(key, index) {
          ...
          return p`<table><thead><tr><th>${key}</th></tr></thead><tbody>
            ${facts_map[key].map((terms) => {

              return p`<tr>
                ${terms.map((term) => p`<td>${term}</td>`)}
              </tr>`
            })}
          </tbody></table>`
        })
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL(new URL('assets/biscuit_bg-2dc195cb.wasm', import.meta.url).href, import.meta.url);
    }
    const imports = {};
    imports.wbg = {};
    ...
    imports.wbg.__wbg_log_2869289a9a809c2a = function(arg0, arg1) {
    ...
    imports['./snippets/biscuit-auth-682e1221039e3171/inline0.js'] = __wbg_star0;
